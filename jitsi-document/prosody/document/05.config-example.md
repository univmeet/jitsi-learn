...
以下是[Prosody源码发布中包含的示例配置文件：prosody.cfg.lua.dist](https://hg.prosody.im/trunk/file/0.12.3/prosody.cfg.lua.dist)。根据主[配置](https://prosody.im/doc/configure)页面中的选项列表调整和修改配置。

> 警告：
> 
> 如果可能的话，建议使用平台安装包提供的配置文件，因为这些文件可能包含配置好的平台特定的默认选项。
...
--
-- 很显然，--用于注释，Prosody会忽略注释行的所有文本。
--
-- 配置分为几个部分，一个全局部分，以及提供主机时定义的各个主机部分。可以添加任意数量的主机部分。
--
-- 列表应该写成：
_ = { "like", "this", "one" }
-- 列表也可以是数字：{ 1, 2, 3 }，等等，逗号或分号可以用作分隔符。
--
-- 表是一个值的列表，每个值都有一个名称，示例如下：
example_ssl = { key = "keyfile.key", certificate = "certificate.crt" }
--
-- 空白（也就是：制表符、空格、换行符）基本上都是没有意义的，可以放在任何地方。
--
...
-- 这些模块时自动加载的，如果希望禁用这些模块，那么需要去掉这里的注释
modules_disabled = {
    -- "offline"; -- 存储离线消息
    -- "c2s";     -- 处理客户端连接
    -- "s2s";     -- 处理服务器到服务器的连接
...
log = {
    info = "prosody.log"; -- 要获取详细的日志，需要把info修改为debug
    error = "prosody.err";
