...
for confpair in mod-http-xslt-filter.conf:50-mod-http-xslt-filter.conf  ; do
	from=$(echo $confpair | cut -d: -f1)
	to=$(echo $confpair | cut -d: -f2)

    # 删除：/etc/nginx/modules-enabled/50-mod-http-xslt-filter.conf
	...
	# 添加符号连接：/etc/nginx/modules-enabled/50-mod-http-xslt-filter.conf
	# 1、全新安装
	# 2、自动删除后重新安装（保留管理员操作）
